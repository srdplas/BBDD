...
-- VISTAS --
/* 11 Cree una vista llamada Nombre_Empleados con el nombre y apellidos (en un solo campo
nombre) de todos los empleados que son de Málaga.*/

CREATE VIEW Nombre_Empleado as select concat(nombre," ", apellido1," ", apellido2)as nombre where ciudad like 'Malaga';

/* 12 Cree otra vista llamada Información_Empleados que contenga el nombre y apellidos (en
un solo campo nombre) y edad (no fecha de nacimiento) de cada empleado.*/
CREATE or replace VIEW Informacion_Empleados as 
select concat(nombre," ", apellido1," ", apellido2)as nombre, round(datediff(curdate(), fecha_nac)/365, 0)as edad 
from empleados;

/* 13 Cree otra vista sobre la anterior llamada Información_Actual que dispone de toda la información de Información_Empleados
junto con el salario que está cobrando en este momento.    */
CREATE OR REPLACE VIEW INFORMACION_ACTUAL AS 
SELECT concat(nombre," ", apellido1," ", apellido2)as nombre, round(datediff(curdate(), fecha_nac)/365, 0)as edad, SALARIO 
FROM EMPLEADOS JOIN HISTORIAL_SALARIAL ON HISTORIAL_SALARIAL.EMPLEADO_DNI=EMPLEADOS.DNI;

/*  Inserte una tupla sobre cada una de estas vistas. ¿Qué sucede?  */
--  QUE NO SE PUEDE EL INSERT INTO ES SOLO PARA TABLAS